...
\begin{figure} 
  \begin{center}
    \begin{overpic}[width = 0.9\columnwidth]{figs/effective_window_geom.png}
	\footnotesize
    \put(54,13){$\vxo^*(\vx)$}
    \put(40,30){$\vxref(\vxo^*)$}
    \put(28,39){$\vx$}
    \put(55,4){\textcolor[RGB]{162,50,52}{$L_{\mathrm{eff},1}$}}  
    \put(70,11){\textcolor[RGB]{162,50,52}{$L_{\mathrm{eff},2}$}}  
...
The expression states that the freuqency band of synthesis is maximal for frontal positions $(\hat{k}_{\mathrm{n}}(\vxo^*)) = 1$, and for sources and receivers far from the SSD. 
...
\subsection{Cutoff freuqency for simple WFS geometries}
...
\begin{multline}
A^{D\cdot P}(\vx,\vxo) = w(\vxo) \, \sqrt{\frac{|\vxref(\vxo)-\vxo| }{|\vxref(\vxo)-\vxo| + |\vxo - \vxs|}}  \cdot
 \\  \frac{\left<\vxo-\vxs \cdot \mathbf{n}(\vxo) \right>}{|\vxo-\vxs|^{3/2}}  \frac{1}{|\vx-\vxo|},
\end{multline}
from which---once the reference curve is defined---the effective lengthes can be calculated according to \eqref{eq:Leff}.
...
\begin{equation}
  ...
  \frac{|\vx-\vxo^*|\,|\vxo^* - \vxs| }{|\vx-\vxo^*| + |\vxo^* - \vxs|}
  \frac{1}{
   \hat{L}_{\mathrm{eff},1/2}(\vx) ^2 \hat{k}_{\mathrm{n}}(\vxo^*)^2  }.
...
In this geometry from simple triangule similarities $\frac{|\vxref(\vxo)-\vxo| }{|\vxref(\vxo)-\vxo| + |\vxo - \vxs|} = \frac{\yref}{\yref - \ys}$ holds for the virtual point source case.
...
The frequency response is normalized by it's ideal high frequency value, given by
\begin{equation}
  \hat{P}_{\mathrm{synth}}(\vx,f) = \frac{P_{\mathrm{synth}}(\vx,f)}{1}
\end{equation}
...
\begin{figure} 
  ...
  \\ \vspace{2mm}
%%
  \begin{overpic}[width = 0.43\columnwidth ]{figs/Fc2_lin_vs_sourcepos.png}
	\scriptsize
	\put(0,0){(c)}
	\end{overpic}
  \hfill
    \begin{overpic}[width = 0.53\columnwidth ]{figs/Fc2_lin_vs_sourcepos_cross.png}
	\scriptsize
	\put(0,0){(d)}
	\end{overpic}
\end{center}
