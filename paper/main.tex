...
We show that the breakdown at low frequencies is governed by the applicability of the stationary phase approximation (SPA) to the 2.5D Kirchhoff integral quantified via an effective window (aperture) length that compresses the influence of SSD size, tapering, referencing, and source/receiver proximity into a single quantity. 
We derive closed-form cutoff conditions for arbitrary source models and listening positions yielding a critical wavenumber that scales with the inverse square of the effective length and recovers classical Fresnel-number/zone theory in frontal directions.
...
Simulations with linear and circular SSDs for plane, point, and focused sources validate the theory, including the intermediate transition band bounded by the two asymmetric effective lengths. 
\end{abstract}
...
Originally introduced by Berkhout in the late 1980s \cite{Berkhout1988, doi:10.1121/1.404755, Berkhout1993:Acoustic_control_by_WFS}, WFS utilizes the Huygens–Fresnel principle to reconstruct intended acoustic wavefronts over an extended listening area. 
...
Consequently, low-frequency performance deteriorates below a geometry-dependent threshold \cite{Spors10ahrens:analysis}. Although this phenomenon is acknowledged within the literature, a comprehensive analytical treatment and rigorous understanding of the precise low-frequency limits remain largely unexplored.

This paper, as the second part of of the series exploring limitations of WFS, specifically addresses these gaps by rigorously examining the low-frequency behavior and limits inherent to WFS. 
...
The remainder of the paper is organized as follows. Section~II revisits the theoretical bases: the problem formulation, the 3D Kirchhoff approximation, the stationary phase approximation (SPA), the derivation of the 2.5D Kirchhoff approximation, and the resulting 2.5D WFS driving functions. 
...
Moreover, an open-source toolbox is available at \href{https://github.com/gfirtha/binauralizer}{\uline{\small{https://github.com/gfirtha/binauralizer}}} for real-time binauralization of virtual WFS systems with adjustable source and receiver position/orientation, enabling listening tests of the present results, highlighting the lower cutoff frequency, effects of tapering, and supporting alternative WFS referencing schemes.
...
\begin{figure} 
  \begin{center}
	...
	\put(8,50){$\Omega$: integral surface}
...
In the context of WFS the surface of integration $\Omega$ is chosen to betranslationally invariant in the vertical direction; its intersection with the horizontal plane is the actual SSD contour (Fig.~\ref{Fig:geometry}). 
...
In the present contribution we are interested only in this latter contribution, with the asymptotic evaluation termed as the stationary phase approximation (SPA)
...
\begin{equation}
  ...
  \label{eq:SPA_series}
\end{equation}
The leading order (geometrical-optics) term is referred to as the stationary phase approximation of the integral, given by \cite{Bleistein1984, Bleistein2000, Bleistein1986b}
{\fontsize{10pt}{\baselineskip}
\selectfont
\begin{equation}
...
Due to the vertical invariance the integral can be split to a vertical and a horizontal integral ($\int_\Omega ... \td S(\vxo) = \int_{C} \int_{-\infty}^{\infty} ... \td z_0 \td s(x_0,y_0)$).
...
Therefore, for virtual fields located in the the synthesis plane with $\hat{k}^P_z = 0$ the vertical stationary position is identically found at $z^* = 0$ as it is illustrated in Figure \ref{Fig:geometry}.
...
The intended field can be estimated by the horizontal SPA of \eqref{eq:25D_KA_gradient} around the integral's stationary points $\vxo^*(\vx)$ that has to be found for each evaluation position $\vx$ (For the sake of brevity this dependency is not denoted in the following), resulting in
{\fontsize{8pt}{\baselineskip}
\selectfont
\begin{multline}
  P(\vx,\omega) \sim 
...
Furthermore, from \eqref{eq:25D_KA_amp_dist} the role of the amplitude correction factor is apparent:
...
The 2.5D WFS driving function for an arbitrary SSD contour can be extracted from the 2.5D KA \eqref{eq:25D_KA}/\eqref{eq:25D_KA_gradient} by making the kernel of the integral independent of the receiver position $\vx$, apparent in $d^{\mathrm{c}}_{h}\left(\vx,\vxo^*\right)$.
...
Again, it is emphasized that the dependency $\vxo^*(\vx)$ is suppressed, but in order to evaluate the above formulation for a given receiver position the stationary SSD element has to be found a-priori as shown in Fig.~\ref{Fig:geometry}.
...
\end{minipage}
  ...
  \caption{Evaluation of a point source's field by the 2.5D approximation at $f_0 = 1~\mathrm{kHz}$ (a), and the measured normalized frequency response at the origin by applying the 3D (b) and the 2.5D Kirchhoff approximation.
...
Written for the 2.5D case this yields 
{\fontsize{8pt}{\baselineskip}
\selectfont
\begin{multline}
...
\selectfont
\begin{multline}
  I_{\mathrm{far}}(\vx,\omega)  =  \sqrt{8\pi \ti k} \oint_C  w(\vxo)  \hkn^P(\vxo)  \,\sqrt{d^{\mathrm{c}}_{v}\left(\vxref(\vxo),\vxo\right) }\cdot
  \\ P(\vxo, \omega)  G(\vx-\vxo,\omega)\, \td s(\vxo).
  \label{eq:Ifar}
\end{multline}}
...
\begin{itemize}
  ...
  Obviously, this error vanishes in case of a planar integration path.
  ...
  For low frequencies, where the SPA does not hold this results in an erroneous low frequency boost for the near field given by $~\sim \sqrt{1 /\ti k}$, and in an overall frequency response of $~\sim \sqrt{\ti k}$ for the farfield component.
...
As the most simple approach the cutoff frequency can be directly estimated from the graph of the synthesized field's frequency response, as shown in Fig.~\ref{fig:Kirchhoff_appr} (b), restricting our investigation to the farfield component $I_{\mathrm{far}}$.
...
\end{multline}}
It is known, that below the unknown cutoff wavenumber $k_{c}$ the slope of the synthesized field decreases by 10 dB/decade ($\sim \sqrt{k}$).
The endpoint of the slope at $k = 0$ is given by 
\begin{equation} 
I_{\mathrm{far}}(\vx, 0) = \oint_C A^{D\cdot P}(\vx,\vxo) \, \td s(\vxo).
\end{equation} 
On the other hand applying piecewise linear approximation to the Bode plot the right endpoint of the slope (which it reaches at $\omega_c = c \cdot k_c$) is the nominal magnitude of the synthesized field given analytically by Eq.~\eqref{eq:WFS_amp_dist}, therefore,
\begin{multline} 
...
\begin{figure} 
  \begin{center}
    \begin{overpic}[width = 0.6\columnwidth  ]{figs/effective_window_simple_geom.png}
    \footnotesize
    \put(80.5,39){$\vxo^*(\vx)$}
    \put(38,39){$\vxref(\vxo^*)$}
    \put(22,39){$\vx$}
...
\end{equation}}
This allows one to express the total phase change up to the end of the equivalent window  $\Delta s = \Leff $ on the cutoff wavenumber $k_c$, which in the present frontal case ($\hkn^P(\vxo^*)=1$) is given by
{\fontsize{8pt}{\baselineskip}
\selectfont
\begin{equation} 
...
The result is in perfect agreement with the theory of Fresnel zones and Fresnel numbers: Eq.~\eqref{eq:omega_c_appr} can be rearranged to
 {\fontsize{10pt}{\baselineskip}
\selectfont
\begin{equation} 
...
As a further generalization let's define the effective window length measured from the stationary position towards both end of the SSD denoted by $\vx_{1/2}$ in Fig.~\ref{Fig:Leff_geometry}.
The effective lengths are calculated from the amplitude distribution of the integrand, normalized by its stationary value, given by
 {\fontsize{10pt}{\baselineskip}
\selectfont
\begin{equation}
...
%\end{equation}}
In order the SPA to hold we may require that the integrand's phase should exceed $\pi / 4$ in both effective window lengths, i.e. two critical wavenumbers are given by 
\begin{equation}
...
Next the application of the previous results is investigated in simple practical WFS scenarios: in case of the synthesis of a point source and a plane wave by using linear and circular SSDs.
...
Once the reference curve is defined the effective lengths can be calculated from the above amplitude distribution according to \eqref{eq:Leff}.
...
\begin{figure}
  ...
  \caption{Synthesis of a point source by using a finite linear SSD segment with total length of 4\,m.
  Fig. (a-c) shows the synthesis of a virtual source at $\vx_{\mathrm{s}}=\posvec{2}{0}{-1}\,\mathrm{m}$ with (b-c) illustrating the frequency response at in the frontal ($\vx_{\mathrm{r}_1}=\posvec{2}{0}{1.5}\,\mathrm{m}$) and an exemplary lateral ($\vx_{\mathrm{r}_2}=\posvec{2}{1.5}{1}\,\mathrm{m}$) receiver position on the reference line.
  ...
  In each cases the estimated lower cutoff frequencies are highlighted by dashed line.}
  \label{Fig:Psynth_linear}
\end{figure}
...
It is reflected that in the present examples of symmetrical SSDs for the frontal position the effective lengths, and, therefore, the two cutoff frequencies coincide.
...
\begin{itemize}
\item Below $\min \{f_{c,1}, f_{c,2} \}$ the SPA does not hold at all: the frequency response changes with $\sim \sqrt{\omega}$ ($+10~\mathrm{dB/dec})$.
It is noted here that this lower critical frequency is sightly underestimated in several extreme cases due to normalization by the stationary point’s amplitude, increasing the one sided apparent effective length (where the stationary point amplitude is significantly smaller then the overall maximum of the amplitude distribution)
\item Above $\max \{f_{c,1}, f_{c,2} \}$ the phase change exceeds $\pi/4$ on both sides of the stationary point, therefore, the SPA holds, and the frequency response sets to its asymptotic high frequency value (with the presence of diffractional artifacts).
This frequency range is the validity range of 2.5D WFS theory.
\item Between the two cutoff frequencies the phase changes sufficiently only on one side of the stationary position (with the longer effective length).
Subsequently, the integrand value can not be considered constant anymore, yet lobes of rapid oscillation do not cancel each other, resulting in a transition frequency band.
Within this transition band the response decreases with $\approx 5~\mathrm{dB/dec}$.
\end{itemize} 

\subsection{Cutoff frequency as function of receiver and source position}

In the following the low frequency limit is investigated as the function of source position and receiver position.
The presented overall low frequency limit is calculated as $\max \{f_{c,1}, f_{c,2} \}$.
...
%%
\end{center}
\caption{Lower frequency limit of 2.5D WFS as the function of receiver position in case of synthesis applying a linear SSD segment (a-b) and a circular SSD (c-e).
Figure (a,c) and (b,d) shows the cutoff frequency vs. receiver position in case of a virtual plane wave and an unfocused point source respectively, while (e) show the virtual focused source case.
...
%  \hfill
  ...
  \hfill
	...
	\end{overpic}
\end{center}
\caption{Lower frequency limit of 2.5D WFS as the function of virtual source position  in case of synthesis applying a a linear SSD segment (a-b) and a circular SSD (c-d).
Figure (a,c) shows the 2D distribution of cutoff frequency with the same color scale as Fig.~\ref{Fig:fc_vs_recpos} while (b,d) shows the same quantity along the frontal position (denoted by dashed line in (a,c)) with varying SSD size.
All the simulations were ran with a Hann window tapering with tapering coefficient of $\alpha = 0.25$, while (d) also shows the effect of applying $\alpha = 0.5$ denoted by dashed line.
}
	\label{Fig:fc_vs_vspos}
\end{figure}


As an important aspect of synthesizing dynamically changing sound fields (e.g. field of a moving source) Fig.~\ref{Fig:fc_vs_vspos} depicts the spatial dependency of the lower cutoff frequency as the function of virtual source position applying a linear and a circular SSD.
...
Similarly for the receiver position, for the linear case the lower cutoff frequency increases rapidly towards lateral directions.
On the other hand, for the circular case the lower cutoff frequency is rotationally invariant around the center due to the synthesis geometry, being an inherent advantage of the circular SSD geometry when synthesizing sources under motion.

In both geometries, the cutoff frequency increases rapidly—and in theory without bound—as the virtual source approaches either the SSD or the receiver, again, resulting in an audible loss of low frequency content. 
At comparable array scales (\(L_{\mathrm{SSD}}\!\approx\! d_{\mathrm{SSD}}\)), a circular SSD yields a higher low-frequency limit for focused sources, whereas a linear SSD is more favorable for sources far from the array. 
The reason is that, in a circular geometry, the effective window length is largest for a focused source: the driving amplitude remains finite along the active arc instead of vanishing toward its endpoints; for a virtual plane wave, the gradient of the virtual field vanishes near the rim, which shortens the effective window. 
The situation reverses for a linear SSD: a plane wave produces an almost constant driving amplitude along the array (hence a long effective window), while for sources close to the SSD the driving decays strongly toward the ends and, due to the focusing operation, the phase varies more slowly along the SSD—both effects reduce the effective window and raise the cutoff frequency.


\section{Conclusion}

This paper established explicit low-frequency validity limits for 2.5D WFS by linking the 2.5D Kirchhoff approximation to the stationary phase approximation (SPA) through an \emph{effective window length} that compresses the impact of SSD size, tapering, referencing, and source/receiver proximity into a single quantity. We derived a closed-form cutoff condition (Eq.~\eqref{eq:low_freq_limit}) that yields two asymmetric critical frequencies \(f_{c,1}\) and \(f_{c,2}\) determined by the effective lengths on either side of the stationary point; the overall validity limit is \(\max\{f_{c,1},f_{c,2}\}\). In frontal directions the result recovers classical Fresnel theory: the SPA becomes valid once the phase increment from the stationary point to the effective window edge reaches \(\pi/4\) (i.e., a half-wavelength across the full effective window), corresponding to a Fresnel number of \(F=0.25\).

The analysis explains and predicts the main trends observed in practice. The cutoff rises steeply for lateral listening positions and for sources or receivers close to the SSD, and it scales inversely with the square of the effective window length. Between the two partial limits, a transition band appears in which the synthesized response is typically \(4\)–\(5\)~dB below the asymptotic level (cf.\ Figs.~\ref{Fig:Psynth_linear} and \ref{Fig:Psynth_circular}). Simulations with linear and circular SSDs for plane, point, and focused sources validate the theory, including the predicted spatial patterns in Figs.~\ref{Fig:fc_vs_recpos} and \ref{Fig:fc_vs_vspos}. At comparable array scales (\(L_{\mathrm{SSD}}\!\approx d_{\mathrm{SSD}}\)), circular SSDs provide higher low-frequency limits for focused sources (long effective window along the active arc), whereas linear SSDs are advantageous for distant/plane-wave–like sources (nearly constant driving and hence a long effective window).

These results offer concrete design guidance: keep sources and listeners away from the SSD where possible; choose SSD size and referencing to maximize the effective window length and the effective distance \(d_h^{\mathrm{c}}\); avoid overly aggressive tapering that shortens the window; prefer circular arrays for moving and focused sources (rotational invariance), and linear arrays for distant or plane-wave reproduction.

Finally, the audibility of the low-frequency loss—especially for lateral conditions—was confirmed by informal listening. The listening test can be \emph{reproduced} with our open-source framework: MATLAB scripts for the figures are available at \href{https://github.com/gfirtha/ieee2025b_firtha/tree/gfirtha/MATLAB}{\uline{\small{https://github.com/gfirtha/ieee2025b\_firtha/tree/gfirtha/MATLAB}}}, and real-time binaural rendering for interactive exploration is provided by \href{https://github.com/gfirtha/binauralizer}{\uline{\small{https://github.com/gfirtha/binauralizer}}}.


\appendix[A local SPA validity criterion]
\label{app:local_SPA}
As a simple local validity condition for the SPA it may be required that the leading order term should dominate over the second one in the integral's expansion around the stationary position.
