...
\thanks{G. Firtha is with the Department of Networked Systems and Services, Faculty of Electrical Engineering and Informatics, Budapest University of Technology and Economics, Műegyetem rkp. 3., H-1111 Budapest, Hungary, E-mail: firtha@hit.bme.hu}
...
Section~IV presents application studies: cutoff estimates for various synthesis geometries; spatial maps versus receiver and source position; and the effects of tapering and referencing. 
Finally, Section~V concludes the contribution.
...
The proposed theoretical framework is initially presented for smooth and potentially enclosing SSDs. 
However, the methodology is readily extendable to non-smooth geometries, such as rectangular arrays, by treating the entire boundary as a single piecewise-linear contour. 
In these cases, the required integrations presented in the following are performed along all contributing segments of the enclosure.
%While such geometries introduce additional local discontinuities at the corners that give rise to edge-diffraction artifacts, the fundamental low-frequency roll-off of the synthesized field remains governed by the total phase increment accumulated across the entire active aperture.

%Consequently, this approach is primarily applicable to smooth or enclosing SSD contours, which represents a realistic requirement for most WFS applications.
%\footnote{For non-smooth geometries, such as rectangular SSDs, the low-frequency limit is evaluated by treating the entire enclosure as a single piecewise-continuous contour. 
%The effective aperture length $\hat{L}_{\mathrm{eff}}$ is then calculated by integrating the amplitude distribution along all contributing segments. 
%While the corners of such distributions introduce local discontinuities that give rise to edge-diffraction artifacts, the underlying low-frequency roll-off remains governed by the phase increment across the total active aperture.}
%\footnote{For non-smooth geometries, such as rectangular SSDs, the low-frequency limit must be evaluated by considering each smooth (linear) segment independently. 
